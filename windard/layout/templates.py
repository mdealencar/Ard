...
class LayoutTemplate(om.ExplicitComponent):
  ...
  def setup(self):
    ...
    self.add_output(
      "x_turbines", np.zeros((self.N_turbines,)), units="m",
      desc="turbine location in x-direction",
    )
    self.add_output(
      "y_turbines", np.zeros((self.N_turbines,)), units="m",
      desc="turbine location in y-direction",
    )
    self.add_output(
      "spacing_effective_primary", 0.0, units="unitless",
      desc="effective spacing in x-dimension for BOS calculation",
    )
    self.add_output(
      "spacing_effective_secondary", 0.0, units="unitless",
