...
class TestGridFarm:
  ...
  def test_setup(self):
    ...
    assert "diameter_rotor" in self.gf.options["modeling_options"]["turbine"]["geometry"].keys()
  ...
  def test_compute_rotatedfarm(self):
    ...
    y_turbines = spacing1*130.0*np.arange(-2, 2+1, 1)
    X, Y = np.meshgrid(x_turbines, y_turbines)
    Xr = np.cos(np.radians(angle_orientation))*X + np.sin(np.radians(angle_orientation))*Y
    Yr = -np.sin(np.radians(angle_orientation))*X + np.cos(np.radians(angle_orientation))*Y

    assert np.all(np.isclose(self.prob.get_val("gridfarm.x_turbines"), Xr.flatten()))
    assert np.all(np.isclose(self.prob.get_val("gridfarm.y_turbines"), Yr.flatten()))
  ...
  def test_compute_skewedfarm(self):
    ...
    assert np.all(np.isclose(self.prob.get_val("gridfarm.x_turbines"), Xs.flatten()))
    assert np.all(np.isclose(self.prob.get_val("gridfarm.y_turbines"), Ys.flatten()))
  ...
  def test_compute_rotatedskewedfarm(self):
    ...
    Xr = np.cos(np.radians(angle_orientation))*Xs + np.sin(np.radians(angle_orientation))*Ys
    Yr = -np.sin(np.radians(angle_orientation))*Xs + np.cos(np.radians(angle_orientation))*Ys
